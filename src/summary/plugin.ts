/**
 * Forked from https://github.com/perspective-community/perspective-summary
 * 
 * 
 *                                  Apache License
   ...
   Copyright 2017 The Perspective Authors
...
const THEMES: { default: string; minimal: string; modern: string } = {
  default: DEFAULT,
  minimal: MINIMAL,
  modern: MODERN,
};
...
type PluginConfig = {
  ...
  format: { [col: string]: number | string };
  header_class: string;
  data_class: string;
  header_classes: { [col: string]: string };
  data_classes: { [col: string]: string };
};
...
export class PerspectiveViewerSummaryPluginElement extends HTMLElement {
  ...
  private _data: null | object;
  private _schema: null;
  private _container: null | HTMLDivElement;
  private _style: null | HTMLStyleElement;
  _loaded: boolean;
  private _shadow: null | ShadowRoot;
  private _global_style: null | HTMLStyleElement;
  constructor() {
    super();
  ...
  async activate(_: View): Promise<void> {
    // nothing to do
  }
  ...
  restore(config: PluginConfig): void {
    this.format(config);
  }

  validate(restore: PluginConfig): void {
  ...
  format(restore: PluginConfig): void {
    if (!this._loaded) {
      return;
    }
    ...
    const is_pivoted =
      null !== this._data &&
      "__ROW_PATH__" in this._data &&
      Array.isArray(this._data.__ROW_PATH__) &&
      this._data.__ROW_PATH__.length > 0;
    ...
    columns?.forEach((col) => {
      ...
      if (is_pivoted && null !== this._data) {
        ...
        const aggregate =
          (aggregations && aggregations[col]) ||
          (Number.isNaN(datum) ? "count" : "sum");
        ...
        if (["integer"].indexOf(this._schema?.[col] ?? "") >= 0) {
          ...
          if (this._config.plugin_config.theme === "modern") {
            ...
            datum = `${+datum < 0 ? "-" : ""}${new Intl.NumberFormat(
              undefined,
              {
          ...
          if (this._config.plugin_config.theme === "modern") {
            datum = new Intl.NumberFormat(undefined, {
          ...
          ["datetime", "date"].indexOf(this._schema?.[col] ?? "") >= 0
        ) {
          // format
          if (this._config.plugin_config.theme === "modern") {
            datum = dayjs(+datum).format(formatter?.toString() ?? "MMM D");
          } else {
            datum = dayjs(+datum).format(formatter?.toString());
          }
        } else {
    ...
    // and refill
    _entries.forEach((value, _) => {
      this._container?.appendChild(value);
    });
  }

  async restyle(_: View): Promise<void> {
    // TODO
  }
